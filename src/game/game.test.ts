...
import {advanceLevel,ALPHABET,consume,createGame,snapshot,spendLetters,useLetters,VOWELS} from './state'
...
describe('rule engine',()=>{
 ...
 it('maps the nine levels from 2 by 2 through 10 by 10',()=>expect(Array.from({length:9},(_,i)=>boardSizeForLevel(i+1))).toEqual([2,3,4,5,6,7,8,9,10]))
 it('spends letters without resetting them during a level',()=>{const p=spendLetters(player(['A']),['A']);expect(p.available).toEqual([]);expect(p.cycle).toBe(1);expect(p.vowelCycle).toBe(1)})
 it('advances with a fresh alphabet while preserving score',()=>{const g=createGame();g.players[0].score=4;g.players[0]=spendLetters(g.players[0],['A','D','H','O']);const next=advanceLevel(g);expect(next.level).toBe(2);expect(next.board).toHaveLength(3);expect(next.players[0].available).toEqual(ALPHABET);expect(next.players[0].score).toBe(4)})
 it('has a strict-dictionary legal solution for the first 2 by 2 level',()=>{const b=createBoard(2),p=player();const first=validateMove(b,p,{row:0,col:0,direction:'H',text:'AD'},strictDict);expect(first.valid).toBe(true);for(const x of first.placements.filter(x=>x.isNew))b[x.row][x.col]={letter:x.letter,owner:0,moveId:1};const p2=spendLetters(p,first.placements.filter(x=>x.isNew).map(x=>x.letter));const second=validateMove(b,p2,{row:1,col:0,direction:'H',text:'HO'},strictDict);expect(second.valid).toBe(true);expect(second.words.map(x=>x.word).sort()).toEqual(['AH','DO','HO'])})
 it('has a strict-dictionary legal solution for the 3 by 3 level',()=>{const b=createBoard(3);let p=player(),moveId=1;for(const input of [{row:0,col:0,direction:'H' as const,text:'ABS'},{row:0,col:0,direction:'V' as const,text:'ACT'},{row:0,col:2,direction:'V' as const,text:'SPY'},{row:0,col:1,direction:'V' as const,text:'BUR'}]){const result=validateMove(b,p,input,strictDict);expect(result.valid,result.errors.join(', ')).toBe(true);const fresh=result.placements.filter(x=>x.isNew);for(const x of fresh)b[x.row][x.col]={letter:x.letter,owner:0,moveId};p=spendLetters(p,fresh.map(x=>x.letter));moveId++}expect(b.every(row=>row.every(Boolean))).toBe(true)})
